...
type raftSet struct {
	...
	Cofiguration  raft.Configuration
}
...
func getRaftSet(num int) []*raftSet {
	...
	// configuration needs to be consistent between services
	// and so we need the full serverlist in this case
	for _, r := range rs {
		r.Cofiguration = raft.Configuration{Servers: servers}
	}
	return rs
}
